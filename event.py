...
def caln_sequence(X):
    ...
    X_mean = np.mean(X)
    X_var = np.var(X)
    X_sc = np.mean((X - X_mean) ** 3)
    X_ku = np.mean((X - X_mean) ** 4) / pow(X_var, 2)
    if (math.isnan(X_ku)):
        print 'error data'
        X_ku = 0
...
for i in range(0,len(list)):
    ...
    if os.path.isfile(path) and list[i].split('.')[-1] == 'ext':
        ...
        if (status == 'horizontal' and ring == 'index3'):
...
for i in range(0,len(list)):
    ...
    if os.path.isfile(path) and list[i].split('.')[-1] == 'ext':
        ...
        if (name not in name_set):
            name_set.append(name)
        if (status == 'negative' and ring == 'index3'):
            file_set.append(path)

info, data = utils.input(file_set, 7, 2) # 10 frames
...
for leave in range(len(name_set)):
    ...
    if (len(X_test) == 0):
        ...
        print name_set[leave], precision, recall
